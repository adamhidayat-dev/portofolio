...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    const colorPalettes = [
      { start: "#2A2415", end: "#14120B", accent: "#E5C158", text: "#FEF8E8" },
      { start: "#132838", end: "#09141D", accent: "#38BDF8", text: "#F0F9FF" },
      { start: "#261638", end: "#12091D", accent: "#C084FC", text: "#FAF5FF" },
      { start: "#0F2C20", end: "#071610", accent: "#34D399", text: "#ECFDF5" },
      { start: "#35161F", end: "#190A0E", accent: "#FB7185", text: "#FFF1F2" },
      { start: "#2D2010", end: "#150E07", accent: "#FBBF24", text: "#FFFBEB" },
    ]

    const drawTile = (img: HTMLImageElement | null, item: MenuItem, i: number) => {
      ...
      if (img && img.complete && img.naturalWidth > 0) {
        ...
        let dy = 0
        if (imgAspect > 1) {
          dw = cellSize * imgAspect
          dx = -(dw - cellSize) / 2
        } else {
          dh = cellSize / imgAspect
          dy = -(dh - cellSize) / 2
        }
        ctx.drawImage(img, dx, dy, dw, dh)
      } else {
        // Ultra-minimalist dark matte background with refined gradient
        const bgGrad = ctx.createLinearGradient(0, 0, cellSize, cellSize)
        bgGrad.addColorStop(0, "#1a1a1a")
        bgGrad.addColorStop(1, "#0d0d0d")
        ctx.fillStyle = bgGrad
        ctx.fillRect(0, 0, cellSize, cellSize)

        // Subtle thin accent circle framing
        ctx.strokeStyle = "rgba(254, 248, 232, 0.08)"
        ctx.lineWidth = 1.5
        ctx.beginPath()
        ctx.arc(cellSize * 0.5, cellSize * 0.5, cellSize * 0.42, 0, Math.PI * 2)
        ctx.stroke()

        // Clean subtle inner ring
        ctx.strokeStyle = "rgba(254, 248, 232, 0.04)"
        ctx.lineWidth = 1
        ctx.beginPath()
        ctx.arc(cellSize * 0.5, cellSize * 0.5, cellSize * 0.26, 0, Math.PI * 2)
        ctx.stroke()

        // Crisp, ultra-clean index typography
        ctx.fillStyle = "#FEF8E8"
        ctx.font = "800 52px system-ui, -apple-system, sans-serif"
        ctx.textAlign = "center"
        ctx.textBaseline = "middle"
        const numStr = i + 1 < 10 ? `0${i + 1}` : `${i + 1}`
        ctx.fillText(numStr, cellSize * 0.5, cellSize * 0.5)
        ctx.textAlign = "left"
        ctx.textBaseline = "alphabetic"
      }

      // Delicate outer edge border
      ctx.strokeStyle = "rgba(254, 248, 232, 0.12)"
      ctx.lineWidth = 2
    ...
    this.items.forEach((item, i) => drawTile(null, item, i))
      ...
      {activeItem && (
        <>
          ...
          <div onClick={handleButtonClick} className={`action-button ${isMoving ? "inactive" : "active"}`}>
            <svg
              className="action-button-icon w-6 h-6 stroke-current"
              fill="none"
              stroke="currentColor"
              strokeWidth="2.5"
              viewBox="0 0 24 24"
